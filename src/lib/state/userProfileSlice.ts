...
import {
  handshakeInstallProtocol,
  handshakeProtocol,
} from "@/data/handshakeProtocolDefinition";
interface Profile {
  name: string;
  bio: string;
  data: any | null;
...
export const createUserProfile = createAsyncThunk(
  ...
  async (
    data: {
      web5: any;
      did: string;
      profile: Omit<Profile, "status" | "error" | "data">;
    },
    { rejectWithValue }
  ) => {
...
export const updateUserProfile = createAsyncThunk(
  ...
  async (
    data: {
      web5: any;
      did: string;
      profile: Partial<Omit<Profile, "status" | "error" | "data">>;
    },
    { rejectWithValue }
  ) => {
...
export const deleteUserProfile = createAsyncThunk(
  ...
  async (data: { web5: any; did: string }, { rejectWithValue }) => {
    ...
    try {
      ...
      if (response.status.code === 202) {
        console.log(
          `Deleted profile for ${profileName}. Status: ${response.status.code}`
        );
        return {
          success: true,
          message: `Profile for ${profileName} deleted successfully`,
        };
      } else {
        throw new Error(
          `Failed to delete profile. Status: ${response.status.code}`
        );
      }
    } catch (error) {
