...
const SendMoneyContent = () => {
  ...
  const [rfqLoaidng, setRfqLoading] = useState(false);
  ...
  const [payoutKeys, setPayoutKeys] = useState([]);
  const [payoutValues, setPayoutValues] = useState(
    payoutKeys.reduce((acc, key) => ({ ...acc, [key]: "" }), {})
  );

  // Handle changes in form fields
  const handlePayoutDataChange = (e: any) => {
    const { name, value } = e.target;
    setPayoutValues({ ...payoutValues, [name]: value });
  };

  const [payinKeys, setPayinKeys] = useState([]);
  const [payinValues, setPayinValues] = useState(
    payoutKeys.reduce((acc, key) => ({ ...acc, [key]: "" }), {})
  );

  // Handle changes in form fields
  const handlePayinDataChange = (e: any) => {
    const { name, value } = e.target;
    setPayinValues({ ...payinValues, [name]: value });
  };
  ...
  const handleGetOffers = (pfi: any) => {
    ...
    setIsOpen(!isOpen);
    setPayoutKeys(
      pfi.offerings[0]?.data?.payout.methods[0].requiredPaymentDetails?.required
    );
    setPayinKeys(
      pfi.offerings[0]?.data?.payin.methods[0].requiredPaymentDetails?.required
    );
  };
  ...
  const requestQuote = async () => {
    ...
    try {
      ...
      console.error("Error signing RFQ", error);
      setRfqLoading(false);
      return;
    }
    ...
    try {
      ...
      alert("Quote requested successfully, now you can proceed to place order");
    }
  ...
  const processQuote = async () => {
    ...
    try {
      ...
      alert("Order placed successfully, Now Close the request");
    }
  ...
  const closeOrder = async () => {
    ...
    try {
      ...
      dispatch(setRfq(null));
      dispatch(setSelectedCredentials(null));
      dispatch(setSelectedPfi(null));
      setStep(1);
      setProccessingOrder(false);
    }
    setProccessingOrder(false);
        ...
        <div className="w-full space-y-4">
          ...
          <div className="relative w-full rounded-lg overflow-hidden border-2 border-black p-3">
            ...
            <div className="absolute h-full inset-y-0 right-0 flex flex-col items-center justify-center p-2 bg-secondar-yellow">
              ...
              >
                {currenciesList
                  .filter((currency) => currency.code !== currencyCodes.payin) // Filter out the selected payin currency
                  .map((currency, index) => (
                    <option key={index} value={currency.code}>
                      {currency.name}
                    </option>
          ...
          <div className="w-full flex flex-row items-center justify-between">
            <p className="text-[14px] font-semibold text-black/70">Fee</p>
            ...
            name ? (
              jwt ? (
                                ...
                                >
                                  ...
                                  {pfi.offerings.map(
                                    (offering: any, index: number) => (
                                      <div
                                        key={index}
                                        className="relative p-2 bg-green-50 rounded-md w-full flex flex-col items-start justify-start space-y-2"
                                      >
                ...
                <button
                  ...
                  onClick={() => verifyUser()}
                >
                  {verifyingUser ? "Verifying ..." : "Verify your account"}
                </button>
              )
            ) : (
      ...
      {step === 2 && (
        <div className="w-full space-y-5">
          ...
          <p className="text-black/70 text-[14px]">
            Now that you&apos;ve found an offering that meets your needs, you
            can request a quote to receive a formal offer.
          </p>

          {payinKeys?.map((field, index) => (
            <div
              key={`${index}+${field}`}
              className="flex flex-col items-start justify-between space-y-1 border-2 border-black p-2 rounded-lg overflow-hidden"
            >
              ...
              <input
                ...
                placeholder={`Enter Sending ${field}`}
                value={payinValues[field]}
                min={0}
                onChange={handlePayinDataChange}
              />
            </div>
          ))}

          {payoutKeys?.map((field, index) => (
            <div
              key={index}
              className="flex flex-col items-start justify-between space-y-1 border-2 border-black p-2 rounded-lg overflow-hidden"
            >
              ...
              <input
                ...
                name={field}
                className="text-md font-medium w-full outline-none border-none text-primary-green"
                placeholder={`Enter reception ${field}`}
                value={payoutValues[field]}
                min={0}
                onChange={handlePayoutDataChange}
              />
            </div>
          ))}
          ...
          {!rfq && (
            ...
            >
              {rfqLoaidng ? "Requesting ..." : "Request Quote"}
            </button>
          )}
          ...
          {rfq && !order && (
            ...
            >
              {proccessingOrder ? "proccessing ..." : "Place Order"}
            </button>
          )}
          ...
          {rfq && order && (
            <button
              ...
              onClick={() => closeOrder()}
            >
              {proccessingOrder ? "proccessing ..." : "Close"}
            </button>
          )}
